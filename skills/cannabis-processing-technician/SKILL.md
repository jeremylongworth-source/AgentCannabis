---
name: cannabis-processing-technician
description: "Coordinate AgentCannabis member workflows for cannabis processing technician; review assistance only."
license: MIT
metadata:
  family: "professional"
  tier: "composition"
  ...
  responsible_role: "cannabis processing technician"
...
Use this professional skillset when the user asks for cannabis processing technician support involving processing records, material intake, batch records, control measures, and deviation evidence. Select relevant member workflows from [the member index](references/member-index.json) and apply only the parts needed for the supplied records.
...
Licence requirements depend on the selected member workflow and the actual site evidence. The responsible review role for this composition is cannabis processing technician; regulated approvals, filings, release decisions, recall decisions, and engineering conclusions remain with authorized humans or qualified professionals.
...
Return a cannabis processing technician review brief with selected member workflows, evidence status, gaps, conflicts, source freshness, risk route, and responsible human next steps. This is not legal certification, engineering approval, lot release, report submission, or authorization to operate.
